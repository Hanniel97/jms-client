...
import { apiRequest, Coordinates, getPlaceDetails, GOOGLE_API_KEY, reverseGeocode, searchPlaces } from "@/services/api";
...
export default function AddCourse() {
  ...
  const { tok, position, prices, setPosition } = useStore();
  ...
  const getPosition = useCallback(async () => {
    ...
    try {
      ...
      setPosition({ latitude: location.coords.latitude, longitude: location.coords.longitude, address: address });
      setPickup(address); // Set pickup address automatically
      setPickupCoords({ latitude: location.coords.latitude, longitude: location.coords.longitude, address: address }); // Set pickup coords
    } catch {
      try {
  ...
  useEffect(() => {
    getPosition(); // Fetch location as soon as the component is mounted
  }, []);
  ...
  // Fonction pour calculer la distance avec Google Maps
  const calculateGoogleDistance = async (lat1: number, lon1: number, lat2: number, lon2: number): Promise<{ distance: number; duration: number }> => {
    try {
      const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${lat1},${lon1}&destination=${lat2},${lon2}&key=${GOOGLE_API_KEY}&mode=driving`;
      ...
      // La distance est en mètres, conversion en km
      const distanceInMeters = data.routes[0].legs[0].distance.value;
      const distanceInKm = distanceInMeters / 1000;

      // La durée est en secondes, conversion en minutes
      ...
      console.error("Erreur Google Directions:", error);
      throw error;
    }
  };

  console.log("Prices from store:", prices);

  const calculateFare = (vehicleType: string, distance: number, prices: any[]): number => {
    const vehicle = prices.find((v) => v.vehicleType === vehicleType);
    if (!vehicle) {
      throw new Error("Type de véhicule invalide");
    }

    const { baseFare, perKmRate, minimumFare } = vehicle;
    const calculatedFare = baseFare + distance * perKmRate;
    const fare = Math.max(calculatedFare, minimumFare);

    return Math.round(fare / 500) * 500; // Arrondir au 500 supérieur
  };
  ...
  const checkDistanceAndPrice = useCallback(async () => {
    ...
    // Utilisez Google Maps pour calculer la distance et la durée
    try {
      const { distance, duration } = await calculateGoogleDistance(lat1, lon1, lat2, lon2);
      ...
      // Calculer le tarif pour chaque type de véhicule
      const fareEco = calculateFare("eco", distance, prices);
      const fareConfort = calculateFare("confort", distance, prices);

      setCourseDetails({
        distance,
        estimatedDuration: `${Math.round(duration)} min`, // Durée estimée en minutes
        fare: {
          eco: fareEco,
          confort: fareConfort,
        },
      ...
      console.log("Distance:", distance, "km, Durée:", duration, "min, Tarif eco:", fareEco, "XOF",  "Tarif confort:", fareConfort, "XOF");
  ...
  }, [dropCoords, pickupCoords, selectedCar, prices]);
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (res.success === true) {
        console.log("Course créée:", JSON.stringify(res.ride));
        close();
                ...
                <View className="flex-1 justify-center px-2 py-2">
                  ...
                  <View className="flex-row justify-between items-center mt-1">
                    <Text className="text-gray-500 font-['RubikRegular']" style={{ fontSize: baseSize(12) }}>
                      {courseDetails?.estimatedDuration ?? (pricingLoading ? "..." : "--")}
                    </Text>
...
// import { CustomLocationTextInput } from "@/components/CustomTextInput";
// import MapPickerModal from "@/components/MapPickerModal";
// import { apiRequest, Coordinates, getPlaceDetails, GOOGLE_API_KEY, searchPlaces } from "@/services/api";
// import useStore from "@/store/useStore";
// import { showError, showSuccess } from "@/utils/showToast";
// import { Icon } from "@rneui/base";
// import { router } from "expo-router";
...
//   const { tok, position, prices } = useStore();
...
//   // Fonction pour calculer la distance avec Google Maps
//   const calculateGoogleDistance = async (lat1: number, lon1: number, lat2: number, lon2: number): Promise<number> => {
//     try {
//         const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${lat1},${lon1}&destination=${lat2},${lon2}&key=${GOOGLE_API_KEY}&mode=driving`;
...
//         // La distance est dans la première route retournée (en mètres), conversion en km
//         const distanceInMeters = data.routes[0].legs[0].distance.value;
...
//     // Utilisez Google Maps pour calculer la distance et la durée
//     try {
//       const distance = await calculateGoogleDistance(lat1, lon1, lat2, lon2);
//       const minDistance = 0.5;
//       const maxDistance = 50;
