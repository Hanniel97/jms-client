                    ...
                    >
                        <BottomSheetScrollView>
                            {rideData?.status === RideStatus.SEARCHING_FOR_RIDER ? (
                                ...
                                <LiveTrackingSheet car={car} rating={rating} duration={Math.max(1, Math.round(duration as number))} item={rideData} />
                            )}
                        </BottomSheetScrollView>
                    </BottomSheet>
                </>
...
//             prev.fare !== next.fare ||
//             prev.updatedAt !== next.updatedAt ||
//             // si la géométrie de route change (recalcul serveur), on veut rafraîchir
//             JSON.stringify(prev?.routeGeometry?.coordinates?.[0]) !== JSON.stringify(next?.routeGeometry?.coordinates?.[0]) ||
//             JSON.stringify(prev?.routeGeometry?.coordinates?.slice(-1)[0]) !== JSON.stringify(next?.routeGeometry?.coordinates?.slice(-1)[0])
//         );
//     }, []);
...
//     const handleRideData = useCallback(
//         (data: any) => {
//             console.log('csvsd data', data.ride.routeGeometry);
...
//                         bottomSheetHeight={mapHeight}
//                         height={mapHeight}
//                         // bottomSheetHeight={HALF_HEIGHT}
//                         // height={HALF_HEIGHT}
//                         status={rideData?.status}
//                         // ⬇️ Géométrie du backend prioritaire
//                         // routeGeometry={rideData?.routeGeometry ?? null}
//                         routeGeometry={rideData?.routeGeometry ?? null}
//                         // ⬇️ ETA & distance du backend si présents
//                         serverEtaMin={rideData?.estimatedDuration}
//                         serverEtaText={rideData?.estimatedDurationFormatted}
//                         serverDistanceKm={rideData?.distance}
//                         drop={{
//                             latitude: toNum(rideData?.drop?.latitude, 0),
//                             longitude: toNum(rideData?.drop?.longitude, 0),
//                             address: rideData?.drop?.address,
...
//                     <BottomSheet
//                         ref={(r) => (bottomSheetRef.current = r as any)}
//                         // index={0} // force la première position
//                         // snapPoints={[HALF_HEIGHT]} // juste une seule taille
//                         // enablePanDownToClose={false} // interdit drag vers le bas
//                         // enableOverDrag={false} // interdit drag vers le haut
...
//                         height={mapHeight}
//                         status={rideData?.status}
//                         routeGeometry={rideData?.routeGeometry}
//                         drop={{
//                             latitude: toNum(rideData?.drop?.latitude, 0),
//                             longitude: toNum(rideData?.drop?.longitude, 0),
//                         }}
